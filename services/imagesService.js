...
export const getImageBuffer = async (url) => {
	try {
		...
		console.warn(
			`⚠️ Error al descargar imagen de Tienda Nube: ${err.message}`
		);
	}
};
...
export async function cropImageAndAddMargin(buffer, margin) {
	...
	console.log("Buffer luego del proceso de crop dentro de la funcion:")
	await waitingConfirmation(finalBuffer);	
	return finalBuffer;
}
